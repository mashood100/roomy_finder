...
class PropertyAdsFilterScreen extends StatefulWidget {
  const PropertyAdsFilterScreen({super.key, this.oldFilter});
  final Map<String, dynamic>? oldFilter;

  @override
  State<PropertyAdsFilterScreen> createState() =>
      _PropertyAdsFilterScreenState();
}

class _PropertyAdsFilterScreenState extends State<PropertyAdsFilterScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            _FilterListile(
              label: "Preferred rent period",
              value: filter["preferredRentType"] ?? "All",
              onTap: () async {
                var val = await Get.to(() => ValueSelctorScreen(
                      title: "Preferred rent period",
                      data: const [
                        "All",
                        "Monthly",
                        "Weekly",
                      ...
                      currentValue: filter["preferredRentType"],
                    ));

                if (val != null) filter["preferredRentType"] = val;
                if (val == "All") filter.remove("preferredRentType");
                setState(() {});
              },
            ),
            const Divider(),
